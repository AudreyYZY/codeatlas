...
@lru_cache(maxsize=1)
def _parse():
    """Lazy-init the TSX parser (cached — construction is not free)."""
...
def parse_ts_file(file_path: str, aliases: dict[str, str] | None = None) -> dict:
    """
    ...
    def walk(node):
        ...
        # `export { x } from "./m"` / `export * from "./m"` are dependency
        # edges just like imports; barrel files are made almost entirely of
        # them, so skipping them leaves holes in the dependency graph.
        if node.type == "export_statement":
            src = node.child_by_field_name("source")
            if src:
                src_text = _node_text(src, source_bytes).strip('"').strip("'")
                    ...
                    if child.type == "export_clause":
                        for spec in child.children:
                            if spec.type == "export_specifier":
                                ...
                                if nm:
                                    exported_names.append(_node_text(nm, source_bytes))
                if not exported_names:
                    exported_names = ["*"]
                for nm_text in exported_names:
                    imports.append(
                        {
                            "symbol_name": nm_text,
                            "alias_name": None,
                            ...
                            "line": node.start_point[0] + 1,
                            "is_type_import": _node_has_child_type(node, "type"),
                        }
                    )
